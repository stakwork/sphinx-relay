...
*/
ava_1.default('test-13-tribe3Private: create private tribe, nodes ask to join, reject and accept, delete tribe', (t) => __awaiter(void 0, void 0, void 0, function* () {
...
function tribe3Private(t, node1, node2, node3) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let tribe = yield save_1.createTribe(t, node1, 0, 0, 0, true);
        ...
        const [n1p1, n2p1] = yield get_1.getCheckContacts(t, node1, node2);
        t.truthy(n2p1, 'node2 contact should exist');
        const checkJoin = yield get_1.getCheckNewJoin(t, node1, n2p1.id, tribeId);
        t.truthy(checkJoin, 'join message should have been sent');
        //NODE1 APPROVES NODE2 TO JOIN TRIBE
        const approve = yield msg_1.appRejMember(t, node1, n2p1.id, checkJoin.id, 'approved');
        ...
        const n1check = yield msg_1.checkMessageDecryption(t, node2, tribeMessage.uuid, text);
        t.true(n1check, 'node1 (admin) should have read and decrypted node2 message');
        //NODE3 SHOULD NOT DECRYPT NODE2'S MESSAGE
        const n3check = yield get_1.getFailNewMsgs(t, node3, tribeMessage.uuid);
        t.true(n3check, 'message should not exist');
        //NODE3 LEAVES THE TRIBE
        let n3left2 = yield del_1.leaveTribe(t, node3, tribe);
        t.true(n3left2, 'node3 should leave tribe');
        //NODE3 JOINS TRIBE CREATED BY NODE1
        if (node1.routeHint)
            tribe.owner_route_hint = node1.routeHint;
        let join3 = yield save_1.joinTribe(t, node3, tribe);
        t.true(join3, 'node3 should join tribe');
        //NODE1 CHECK FOR JOIN MESSAGE FROM NODE3
        const checkJoin3 = yield get_1.getCheckNewJoin(t, node1, n3p1.id, tribeId);
        t.truthy(checkJoin3, 'join message should have been sent');
        //NODE1 ACCEPTS NODE3 TO JOIN TRIBE
        const approve2 = yield msg_1.appRejMember(t, node1, n3p1.id, checkJoin3.id, 'approved');
        ...
        //NODE1 (ADMIN) SHOULD DECRYPT NODE2'S MESSAGE
        const n1check3 = yield msg_1.checkMessageDecryption(t, node1, tribeMessage3.uuid, text3);
        t.true(n1check3, 'node1 (admin) should have read and decrypted node2 message');
        //NODE3 SHOULD DECRYPT NODE2'S MESSAGE
        const n3check3 = yield msg_1.checkMessageDecryption(t, node3, tribeMessage3.uuid, text3);
