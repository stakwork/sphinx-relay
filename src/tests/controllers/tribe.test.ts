...
test.serial(
  'Tribe test for seeing that if 2 nodes have the same alias, the sender alias is changed',
...
async function tribeUniqueAliasTest(
  ...
  t.true(
    newNode2.alias !== old_alias,
    'node2 alias should not be equal to old alias'
  )
  t.true(
    newNode2.alias === node1.alias,
    'node2 alias should be equal node1 alias'
  )
  ...
  t.true(
    message1.sender_alias !== message2.sender_alias,
    'The sender alias in both messages should be different'
  )
  //Check that our logic for assigning an alternate alias is working
  t.true(
    message2.sender_alias === `${node1.alias}_2`,
    'The sender alias should be modified according to our unique alias logic'
  )
